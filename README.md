...
**Dependencies (loaded via CDN)** - no install step required:

- `react@18` and `react-dom@18` - production builds (`react.production.min.js`, `react-dom.production.min.js`)
- `@babel/standalone@7` - in-browser JSX transformation

> Note: React is loaded from the production build, so React's development-mode warnings and verbose error messages are not shown in the console.
